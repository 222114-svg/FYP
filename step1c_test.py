"""
STEP 1 — Download Test Videos ONLY
Run this in a separate terminal alongside step1.py (train) and step1_val.py
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("download_test_log.txt", encoding="utf-8"),
...
if __name__ == "__main__":
    log.info(f"SignBridge — Downloading TEST split")
    log.info(f"Dataset dir : {DATASET_DIR}")
    log.info(f"Output dir  : {OUT_DIR}")

    path = Path(DATASET_DIR) / JSON_FILE
    entries = json.loads(path.read_text(encoding="utf-8"))
    log.info(f"Total test entries: {len(entries)}")
    ...
    log.info(f"\nTEST done: OK={ok}  FAIL={fail}  SKIP={skip}")